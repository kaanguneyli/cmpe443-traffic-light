...
#define RCC_AHB1ENR *((volatile uint32_t *)0x40021048)
...
typedef struct {
  ...
  volatile uint16_t
      pedestrian_wait; // waiting duration between the start of red light to
                       // cars and green light to pedestrians
  volatile uint16_t animation_duration;
  volatile uint16_t car_wait; // waiting duration between the start of red light
                              // to pedestrians and green light to cars
} TrafficLight;
